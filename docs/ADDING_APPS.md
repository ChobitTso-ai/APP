...
- `group`：卡片放哪一區。
  - `'mobile'`（📱 手機也能用）——**在手機上就順手好用的工具**放這區。
    做成 PWA 的（自己有 `manifest.webmanifest` + `sw.js`，手機能「加入主畫面」）
    一定放這區；不是 PWA 但主要在手機上操作的也放這區（例如即時投票，
    參與者是用手機掃 QR 作答）。區塊說明寫的是「**有些**還能加入主畫面」，
    所以放非 PWA 的工具進來不會讓文案變成謊話。
  - `'desktop'`（💻 電腦操作）——需要大畫面、處理檔案的工具，沒填時的預設值
