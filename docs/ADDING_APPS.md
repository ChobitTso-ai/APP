...
```js
// 把其中一個 { ...wipSlot } 換成：
{ name: 'App 名稱', desc: '一句話說明（20 字內）', icon: '🦷', url: 'apps/你的-app-代號/index.html', added: '2026-07-26', group: 'desktop' },
```
...
- `added`：上架日期（今天）。**最新上架的 App 會自動掛 🆕 徽章**（60 天後自動消失），
  沒填就不會有徽章。瀏覽次數最高的 App 會自動掛 🔥，不用手動指定。
- `group`：卡片放哪一區。
