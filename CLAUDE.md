...
- 已上線 App：`apps/case-marker/`（案例標記工具 v1.5：IndexedDB 自動保存、
  ...
  **多人即時協作需要後端、
  ...
  `apps/pdf-toolbox/`（PDF工具箱 v2.2：合併、分割、編輯、轉換四個分頁。
  v2.0 合併與分割；v2.1 引入 pdf.js〔legacy UMD，全域 `pdfjsLib`，延遲載入〕
  做頁面縮圖——分割分頁原本每頁只顯示 📄 佔位圖，等於閉著眼睛選分割點，一併修掉；
  新增「編輯」分頁〔遮蔽／旋轉／刪除／依範圍選取／擷取／拖曳重排／頁碼／壓縮〕與
  「轉換」分頁〔圖片轉 PDF、PDF 轉圖片〕。
  **遮蔽是真遮蔽**：用 pdf.js 把該頁畫成點陣圖 → 在 canvas 上塗掉 → 整頁當影像
  重新嵌回去，底層文字真的消失。〔若只用 pdf-lib 畫黑色矩形蓋上去，底下文字原封
  不動留在檔案裡，複製貼上或移掉矩形就看得到——政府／法院文件外洩的經典原因。〕
  預設實心塗黑而非馬賽克：馬賽克在字元集有限時（例如純數字病歷號）有被還原的風險。
  代價是有遮蔽的頁面會變成影像、文字不能再選取，所以只轉有遮蔽的那幾頁。
  遮蔽框存 0~1 相對座標，頁面一旋轉就用 `rotateRect()` 同步換算，不然位置會跑掉。
  頁碼要畫在「看起來的底部」，頁面有 /Rotate 時 PDF 座標系跟視覺方向不一樣，
  `placePageNumber()` 對 0/90/180/270 四種角度各自換算。
  **`pdf.js` 會把傳進去的 ArrayBuffer 轉移給 worker（detached）**，
  ...
  v2.2 遮蔽視窗可縮放平移〔canvas 絕對定位在 `.redact-viewport` 裡，縮放只改它的
  CSS 寬高；座標一律用 `getBoundingClientRect()` 換算成 0~1，所以縮放平移不影響
  框選準確度。滾輪縮放、雙指縮放平移、框選／移動模式切換〕、遮蔽框可單獨選取
  〔點一下選取，可拖曳移動、按鈕或 Delete 刪除——先前只有「復原上一個」與
  「全部清除」，框歪一個要退掉後面全部〕、**縮圖直接畫上黑塊**〔隱私功能不能
  只掛數字徽章，匯出前要看得到自己遮了哪；遮蔽內容納入縮圖快取 key〕、
  未套用就關閉會先確認〔誤點暗色背景原本會靜靜丟掉所有框〕、
  頁面操作有真正的復原〔`pushUndo()` 存快照，上限 30 步，Ctrl+Z；
  先前只有「全部重來」，刪錯一頁得把旋轉遮蔽排序全部重做。
  **新增會改動 `editorPages` 的操作時記得在前面補 `pushUndo()`**〕、
  遮蔽頁解析度可設定〔原本寫死 200 DPI，300 DPI 的掃描件會被降級〕、
  圖片轉 PDF 補上拖曳排序〔**UI 早就寫了「可拖曳排序」但根本沒實作**〕、
  移除合併分頁沒有作用的「添加書籤」勾選框〔v2.0 就在，程式從沒讀過它〕、
  長時間工作顯示「第幾張／百分比／已產生容量」。
  端到端測試在 `tests/pdf-toolbox.test.js`〔68 項〕。最關鍵的幾項：把成品用
  pdf.js 讀回來看文字圖層，確認被遮的字真的不見、沒遮的頁文字仍在；
  「先遮蔽再旋轉」後取樣像素，確認黑塊落在旋轉後的正確位置、原位置沒殘留
  （遮蔽框、渲染、輸出三邊的座標系必須一致）；以及在放大狀態下用滑鼠實際拖框，
  驗證存下來的相對座標與滑鼠位置吻合。
  **寫測試時 `loadIntoEditor` 這類重新載入的輔助函式，要先把 `editorPages`
  清空再等**——否則「已經有 N 頁」在舊資料上就成立，會在載入完成前往下跑，
  後面設的東西被隨後完成的載入蓋掉（踩過）。
  **密碼加密沒做**——pdf-lib 沒有這個功能〔`setPassword`／`encryptWith` 都不存在〕，
  要做得換函式庫；OCR 也沒做〔tesseract.js 中文語言包 ~15MB，手機上太慢〕。
  遮蔽「套用到多頁」經評估暫緩，使用者說先不用）。
